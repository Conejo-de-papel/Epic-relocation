...
// PLAY
  ...
  drawSprite(bird);
    ...
    if(frameCount%100 == 0) {
			pipeImg = loadImage(str(int(random(1,6)))+".png");
      var pipeH = random(0, height/2,5);
      var pipe = createSprite(bird.position.x + width, pipeH);
      pipe.addImage(pipeImg);
			pipe.scale = random(0.1,0.35);
...
// GAME OVER
	...
	if(gameOver){
		...
		fill(255,200);
		text('Epic Relocation', camera.position.x, height/2);
